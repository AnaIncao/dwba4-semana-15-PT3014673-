...
def cadastroContatos():
  try:
    ...
    if flask.request.method == "POST":      
      contatos[flask.request.form['email']] = {'nome': flask.request.form['nome'], 'telefone': flask.request.form['telefone'], 'assunto': flask.request.form['assunto'], 'mensagem': flask.request.form['mensagem'], 'op': flask.request.form['op']}
...
@app.route('/limparResgistro/<email>', methods=['POST'])
def limparResgistro(email):
  try:
    contatos = db.get('contatos', {});
    del contatos[email];
    db['contatos'] = contatos;
